...
def real_trading(request):
    ...
    response = my_common.ApiRequests(my_constant.realtrading_url, "GET")

    id = 1
    length = len(response)
    for i in range(length):
        info = response[length - i - 1]
        body_data.append({'uid': info['id'], 'id': id, 'transactionID': info['transactionID'], 'userId': info['userId'],
                          ...
                          'profit': str(info['profit']), 'swap': str(info['swap']), 'stopLoss': str(info['stopLoss']), 'takeProfit': str(info['takeProfit'])})
        id = id + 1
...
def demo_trading(request):
    ...
    response = my_common.ApiRequests(my_constant.demotrading_url, "GET")
    length = len(response)
    id = 1
    for i in range(length):
        info = response[length - i - 1]
    ...
    return render(request, 'erps_template/trading/demo_trading.html', {
        'bodydata': body_data,
        'userlevel': user_level,
        'hostip': my_constant.api_host,
        'userList': userList,
...
def get_real_trading(request):
    ...
    response = my_common.ApiRequests(my_constant.realtrading_url, "GET", params)
...
def get_demo_trading(request):
    ...
    response = my_common.ApiRequests(my_constant.demotrading_url, "GET", params)
